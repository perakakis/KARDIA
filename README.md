[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.2638856.svg)](https://doi.org/10.5281/zenodo.2638856)
...
## How to cite
Perakakis P., Joffily M., Taylor M., Guerra P., Vila J. (2010). [KARDIA: a Matlab software for the analysis of cardiac interbeat intervals](Documentation/Articles/Perakakis_KARDIA_2010.pdf). Computer Methods and Programs in Biomedicine 98, 83–89.

doi: [10.1016/j.cmpb.2009.10.002](http://dx.doi.org/10.1016/j.cmpb.2009.10.002).
...
When you click ```ok``` a window opens that lets you select the files containing the IBI data that you wish to analyze. After opening the files, KARDIA plots the IBI data for each subject in the first graph of the graph panel. You can scroll through subjects using the arrows or type the name of a subject file in the edit box to jump directly to that subject. Clicking inside the graph opens an editable Matlab figure, giving you more visualization options and the possibility to export the graph in various formats.
...
When you import an event file the number of ```Eventfiles``` in the graph panel is automatically updated.

![Data graph.](Documentation/Figures/DataGraph.png)

In case you want to use the same event file for all subjects, assuming that the same events occur at the same latencies in every recording, import this event file for the first subject in the drop list and continue with the analysis without importing any other event files.
...
When you click ```ok``` KARDIA plots the results in the second graph. Again you can scroll through subjects or see the grand average of all subjects for all conditions. Clicking inside the graph opens a separate Matlab figure.
...
In our example we can choose the ```baseline``` code which marks the beginning of a 5-minute rest period. We select Epoch start ```0``` and Epoch end ```300``` to create the 5-minute epoch. When you click ```ok``` KARDIA plots the selected segments in the third graph. Once again, you can scroll through subjects and conditions using the arrows and open a separate Matlab figure by clicking inside the graph.
...
The last HRV module is used to perform the detrended fluctuation analysis (DFA), an algorithm proposed by [Peng et.al, 1995](Articles/Peng1995.pdf) for the quantification of scaling exponents in non-stationary time series. Here you can select the range of boxes and if you want to use adjacent or overlapping (sliding) windows.
